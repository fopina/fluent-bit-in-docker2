                          ...
                          struct flb_config *config, void *data)
{
    ...
    if (get_cgroup_version(ctx) == CGROUP_V2) {
        flb_plg_info(ctx->ins, "Detected cgroups v2x");
        in_docker_set_cgroup_api_v2x(&ctx->cgroup_api);
        ctx->cgroup_version = CGROUP_V2;
    }
    else {
        flb_plg_info(ctx->ins, "Detected cgroups v1x");
        in_docker_set_cgroup_api_v1x(&ctx->cgroup_api);
